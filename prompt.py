...
#
# 请用单句公式概括故事本质，例如：
# "当[主角]遇到[意外邂逅]，必须[勇敢追求内心渴望]，否则将错失[难得的青春美好]；与此同时，[内心微妙的不安与期盼]悄然浮现。"
#
# 要求：
# 1. 体现温馨细腻的情感冲突与内心矛盾（无须过于激烈）
# 2. 突显人物的情感驱动力与成长历程
# 3. 暗示校园生活中的现实挑战与美好期待
# 4. 使用25-100字精准表达
#
...
# 在原有基础上增加 {previous_chapters_summary} 占位符
chunked_chapter_blueprint_prompt = """\
基于以下元素，并重点参考“先前剧情进展摘要”，继续设计后续章节的蓝图：
- 内容指导：{user_guidance}
- 小说整体架构：
{novel_architecture}

- 先前剧情进展摘要 (基于已写章节正文生成，请优先参考)：
{previous_chapters_summary}

- 当前已有的章节蓝图列表（仅供参考章节号和标题顺序）：
{chapter_list}

需要生成总共{number_of_chapters}章的节奏分布，现在请设计第{n}章到第{m}章的节奏分布：

设计规则：
1.  **紧密衔接**：新生成的第 {n} 章必须能直接承接“先前剧情进展摘要”中描述的最新状态和情节线索。
2.  章节集群划分：
    - 每3-5章构成一个悬念单元，包含完整的小高潮。
    - 单元之间设置"认知过山车"（连续2章紧张→1章缓冲）。
    - 关键转折章需预留多视角铺垫。
3.  每章需明确：
    - 章节定位（角色/事件/主题等）
    - 核心作用（推进/转折/揭示/等，需与剧情摘要和整体架构对应）
    - 悬念密度（紧凑/渐进/爆发/等）
    - 伏笔操作（埋设/强化/回收，需考虑剧情摘要中的线索）
    - 认知颠覆强度（1-5级）
    - 本章简述（一句话概括，清晰连接上一章或摘要的结尾）
...
要求：
- **优先考虑剧情摘要**：确保生成的蓝图是对剧情摘要中情节的合理延续和发展。
- 使用精炼语言描述，每章简述控制在100字以内。
- 合理安排节奏，确保整体悬念曲线的连贯性。
- 在生成{number_of_chapters}章前不要出现结局章节。
...
# =============== 累积摘要更新 (修改后) ===============
summary_prompt = """\
作为一名专业的小说编辑，你的任务是根据“刚刚完成的章节文本”和“之前的累积摘要”，生成并返回**新的、截止到当前章节**的累积摘要。

当前操作针对章节： **第 {current_chapter_number} 章**

**输入信息：**

1.  **上一章节（第 {previous_chapter_number} 章）的累积摘要** (如果这是第一章，则此项为空):
    ```
    {previous_cumulative_summary}
    ```

2.  **当前刚刚完成的章节文本 (第 {current_chapter_number} 章):**
    ```
...
1.  **理解历史背景：** 阅读“上一章节的累积摘要”（如果存在），把握截止到上一章的核心情节和状态。
2.  **分析新进展：** 阅读“当前刚刚完成的章节文本”，提取本章的关键情节进展、人物状态变化、重要线索等。
3.  **整合与更新：** 将本章的新进展（步骤2）与历史背景（步骤1）融合，形成一个连贯、准确、覆盖从第1章到**第 {current_chapter_number} 章**的全新累积摘要。确保新摘要能够自然地衔接历史背景并准确反映本章内容带来的变化。
4.  **精炼与输出：** 新摘要需语言简洁，重点突出，总字数控制在2000字以内。

**输出要求：**
*   **仅返回**新生成的、截止到第 {current_chapter_number} 章的**完整累积摘要文本**。
*   不要包含任何额外的解释、标题、标记或确认信息。

请开始生成截止到第 {current_chapter_number} 章的累积摘要：
"""
...
参考文档：
└── 前文章节累积摘要 (截止至第 {previous_chapter_number} 章)：
    {previous_cumulative_summary}
...
确保章节内容与前文摘要、上一章完整内容、前章结尾段衔接流畅、下一章目录保证上下文完整性，
...
# =============== 新增：为重新生成后续蓝图，对已写章节内容进行摘要 ===================
summarize_written_chapters_prompt = """\
作为一名专业的小说编辑，你需要根据以下已完成的章节正文内容，生成一个简洁、连贯、包含核心情节进展和当前状态的**剧情进展摘要**。这个摘要将用于指导后续章节蓝图的生成。

已完成章节内容：
{combined_chapters_text}

要求：
1.  **聚焦核心进展**：提取关键的情节转折、人物关系变化、新出现的重要线索或冲突、已解决和未解决的问题。
2.  **体现当前状态**：简要说明主要角色目前的目标、处境、心态以及彼此间的关系状态。
3.  **保持连贯性**：摘要应按时间顺序或逻辑顺序组织，形成一个流畅的叙述流。
4.  **简洁精炼**：尽量用最少的文字概括最多的信息，避免不必要的细节描写和对话复述。总字数控制在 500 字以内（根据需要调整）。
5.  **客观中立**：只描述已发生的事实，不进行预测或评价。
6.  **输出格式**：直接输出摘要文本，不需要任何额外标记或解释。
